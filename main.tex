...
\item A context hash $H_{\mathsf{ctx}}$ that binds the protocol instance to an application identifier, the server public keys, and all public generators, preventing cross-deployment replay and parameter-substitution attacks. Specifically, the production formula is:
  \[
  H_{\mathsf{ctx}} = \mathcal{H}_{\mathsf{scalar}}\!\left(
    ...
    \parallel h_0 \parallel h_1 \parallel h_2 \parallel h_3 \parallel h_4
  \right),
  \]
...
\item \textbf{Output:}
  The canonical proof is $\pi_{\text{BEQ}} = (C_{25519}, R_{\text{BLS}}, R_{25519}, z, z_{\text{BLS}}, z_{25519}, \pi_{\text{range}})$.

  \textbf{Transcript compression (reference implementation):}
  The reference implementation omits $R_{\text{BLS}}$ and $R_{25519}$ from the wire format, transmitting instead only the challenge $(c_{128}, C_{25519}, z, z_{\text{BLS}}, z_{25519}, \pi_{\text{range}})$. The verifier reconstructs:
  \[
  R_{\text{BLS}} = z \cdot h_4 + z_{\text{BLS}} \cdot h_0 - c \cdot C_{\text{BLS}}, \quad
...
\item \textbf{TOCTOU ordering: nullifier written last, only after all proofs pass.}
  ...
  $\DB_{\mathsf{spend}}$ only after every cryptographic check passes (Sections
  4.3.3 step~11 and 4.4.3 step~14).  Inserting the nullifier before verifying
...
\item \textbf{Write ordering: idempotency cache written before nullifier insertion.}
...
\item \textbf{Client-side persistence of $k_{\mathsf{daily}}$ and $r_{\mathsf{daily}}$
  before the refresh request.}
  A client that reboots after the server has processed the refresh but before
  the response arrived can reconstruct the identical request only if
  $k_{\mathsf{daily}}$ and $r_{\mathsf{daily}}$ are stored durably beforehand.
  Without them the client must generate a new $K_{\mathsf{daily}}$ and a new
  proof; the server's idempotency cache cannot help because the cache is keyed
  by the proof hash, and the new proof hashes differently.  The result is
  permanent epoch lockout even though the server acted correctly.  The blinding
  factor $r_{\mathsf{daily}}$ is needed to unblind the server's response:
  $s_{\mathsf{daily}} = r_{\mathsf{daily}} + s'_{\mathsf{daily}}$.  This
  makes the persistence requirement cryptographic, not merely operational.
...
\item \textbf{$H_{\mathsf{ctx}}$ binds all generators and both server keys.}
  ...
  also both key types and all five generators (see updated formula in
...
\item \textbf{Transport-layer anonymity is orthogonal to the cryptographic protocol.}
  The EBUT cryptographic layer is fully anonymous, but a static IP address
  that fetches a nonce and then submits a spend request leaks correlation at
  the network layer.  Deployments must choose between anonymous-channel nonce
  retrieval (e.g., Tor) or ZK-membership proofs with token-bound leaf
  nullifiers (Section~7.10) when transport-layer unlinkability is required.

\item \textbf{Epoch boundary burst capacity is an acceptable trade-off.}
  During the overlap of an active grace window a user may hold an unexpired
...
\item \textbf{Fixed-base precomputed scalar multiplication (FixedBaseTable).}
  The five protocol generators $g_1, h_0, \ldots, h_4$ are known at startup.
...
\item \textbf{Batch verification via RLC pairing aggregation and parallel BEQ.}
  \texttt{verify\_refresh\_batch} and \texttt{verify\_spend\_batch} aggregate
  $N$ proofs using per-proof RLC weights $\rho_i$: one Pippenger MSM for all
  Schnorr equations; one two-pair \texttt{multi\_miller\_loop} for all
  pairing equations; and parallel \texttt{rayon::par\_iter} for the
  independent BEQ range proofs.  This reduces
  \texttt{final\_exponentiation} calls from $N$ to $1$.
...
\item[\textbf{SpendProof includes $T_{\mathsf{scale},R}$ and $T_{\mathsf{refund}}$.}]
  Section~4.4.2 notes that the refund bridging pair
  $(T_{\mathsf{scale},R}, T_{\mathsf{refund}})$ carries no additional soundness
  and may be omitted.  The reference implementation computes and includes
  both in the \texttt{SpendProof} struct and in the Fiat--Shamir challenge
  hash, adding two 48-byte group elements to the proof but making no change
  to the security argument.  The paper's optimisation of omitting them may be
  applied to reduce proof size.
...
\item[\textbf{BEQ wire format.}]
  The specification in Section~5.2 lists
  $\pi_{\text{BEQ}} = (C_{25519}, R_{\text{BLS}}, R_{25519}, z, z_{\text{BLS}},
  z_{25519}, \pi_{\text{range}})$.  The production wire format is
...
release mode (\texttt{RUSTFLAGS="-C target-cpu=native"}, enabling AVX2
...
The benchmark source is \texttt{tests/performance.rs}.
...
\hline
Spend proof          & \textbf{1548 B} & BBS+ core: 144 B (3$\times$G1) \\
                     &       & Bridge commitments: 288 B (6$\times$G1) \\
...
\hline
\end{tabular}
\caption{Wire-format proof sizes for each EBUT phase.}
\label{tab:sizes}
\end{table}
...
\hline
Spend: client prove  & 16.2 & 1548 &  62 \\
Spend: server verify &  6.4 & 1548 & 156 \\
\hline
\end{tabular}
\caption{End-to-end timings and single-threaded throughput.  Measured at 10
iterations each, release build with AVX2 enabled.}
\label{tab:e2e}
\end{table}
...
At 124~ops/s for Epoch Refresh verification and 156~ops/s for Spend
...
Epoch Refresh proof generation takes $\approx$19~ms and Spend proof
...
Each full Refresh or Spend proof fits in well under 2~KB (1620~B and
1548~B respectively), making the protocol suitable for TLS-protected
...
Spend at $\approx$3.2~KB.  The reference implementation achieves
...
commitment pair from the wire format.
