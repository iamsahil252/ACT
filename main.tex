...
\item \textbf{Idempotency cache check (FIRST):} Compute
  \[
  \mathtt{key}_{\mathsf{ref}} = \mathcal{H}_{\mathsf{scalar}}\!\bigl(\text{EBUT:Idempotency:Refresh} \parallel \mathsf{len}(\pi_{\mathsf{refresh}}) \parallel \pi_{\mathsf{refresh}}\bigr).
  \]
  If $\mathtt{key}_{\mathsf{ref}}$ is present in the idempotency cache, return the cached $(A_{\mathsf{daily}}, e_{\mathsf{daily}}, s'_{\mathsf{daily}})$ immediately \textbf{without} re-executing the expensive verification or touching $\DB_{\mathsf{epoch}}$.
...
\item \textbf{Nonce read-only check.} Verify that $\eta$ is non-zero, is exactly 16 bytes, and has not yet been marked spent. In standard mode, check a short-lived cache against the exact nonce store; in high-privacy ZK-membership mode (Section~7.10), verify the ZK membership proof against the current Merkle root $\mathsf{rt}$ and check that the leaf nullifier $N_\eta \notin \mathcal{N}_{\mathsf{spent}}$. \textbf{Do not mark $\eta$ (or $N_\eta$) as spent at this step.}
...
\item $\pi_{\text{BEQ\_s}}$ for the hidden spend amount $s$ using BLS generator $h_5$.
\end{itemize}
The request discloses $(\mathsf{mode}=\text{``Hidden''}, k_{\mathsf{cur}}, T_{\mathsf{issue}}, \eta, K', C_{\mathsf{BP}}, C_s)$ and does \emph{not} disclose $s$ or $s_{\mathsf{req}}$.
...
\begin{enumerate}[leftmargin=*]
\item Require $\mathsf{mode} = \text{``Hidden''}$; require $(C_s, \pi_{\text{BEQ\_s}})$ to be present; and reject any disclosed Exact-mode-only fields $(s, s_{\mathsf{req}}, T_{\mathsf{scale},T}, T_{\mathsf{total}})$.
\item Compute the spend idempotency key over the Hidden-mode statement, binding $\text{``Hidden''}$, $C_s$, $\pi_{\text{BEQ\_s}}$, $\pi_{\mathsf{spend}}$, and $\eta$.
...
\item Verify the refund and range bridges using $z_m = z_{\tilde{c}} - z_{\widetilde{a}}$:
  \begin{align*}
    ...
    z_m \cdot h_4 + z_w \cdot h_0 - T_{\mathsf{BP}} &\stackrel{?}{=} z_{r_1} \cdot C_{\mathsf{BP}} - T_{\mathsf{scale},BP}.
  \end{align*}
\item Verify $\pi_{\text{BEQ\_s}}$ with respect to $C_s$ and reject $s=0$ via the proof statement (e.g., an explicit non-zero check or a range on $s-1$).
\end{enumerate}
...
In Exact mode (Section~\ref{sec:spend-exact}), the spend amount $s$ is transmitted in plaintext. While the token nullifiers $k_{\mathsf{cur}}$ are perfectly randomized and unlinkable across spend requests, the metadata value $s$ creates a potential behavioral side-channel. A server that logs spend requests may perform statistical fingerprinting on the sequence of disclosed $s$ values to link intra-epoch transactions originating from the same user. For many deployments this is an acceptable trade-off, because the server is already trusted to enforce the fixed price. Deployments that require amount privacy should instead use Hidden mode (Section~\ref{sec:spend-hidden}), in which the server sees only the commitment $C_s$ and the associated zero-knowledge proofs. Hidden mode removes this metadata channel at the cost of larger proofs and additional verification work.
